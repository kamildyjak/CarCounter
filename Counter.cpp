...
int CarCounter::Counter::start(string src, bool cam, int type) {

	Mat structElement5 = getStructuringElement(MORPH_RECT, cv::Size(5, 5));
	...
	if (!cap.isOpened()) {
		textBox1->Text = "Wyst¹pi³ b³¹d!";
		if (cam) {
			errorBox->Text = "B³¹d odczytu z kamery";
		}
		else {
			errorBox->Text = "B³¹d odczytu pliku";
		}
	...
	while (!frame1.empty()) {
		...
		GaussianBlur(frame1_gray, frame1_gray, cv::Size(5, 5), 0);
		...
		if (debug) {
			...
			//putText(final, "Car count: " + to_string(carCount), cv::Point(20, 40), FONT_HERSHEY_SIMPLEX, 1.4, COLOR_YELLOW, 2);
		}
